...
import { loadMexico } from './data-loader.js'

// Story tab chart (Spanish-only). Two views share one canvas via the toggle:
//   'fuentes'  — Mexico's top import sources, green vs roasted (recent 3 yrs)
//   'colombia' — Colombia's coffee exports to Mexico by year
// The SVG is rebuilt per render (small chart, infrequent renders) so the two
...
const NAME_ES = {
  ...
  GTM: 'Guatemala', IDN: 'Indonesia', GBR: 'Reino Unido', ESP: 'España', SLV: 'El Salvador',
}
...
export function wireMexicoChart() {
  const root = document.getElementById('mexico-chart-canvas')
  if (!root) return
  ...
  const toggle = document.getElementById('mexico-chart-toggle')
  if (toggle) {
  ...
  const ro = new ResizeObserver((entries) => {
    ...
    resizeRaf = requestAnimationFrame(() => {
      ...
      if (!hasBooted) {
        hasBooted = true
        loadMexico().then((d) => { data = d; render() })
      } else if (data) {
        render()
      }
    })
