var RadarChart = {
    ...
    chart: function() {
        function b(b, c) {
            if (0 == c || void 0 == c) b.classed("visible", 0), b.select("rect").classed("visible", 0);
            else {
                b.classed("visible", 1);
        ...
        function c(c) {
            c.each(function(c) {
                ...
                if (t.enter().append("polygon").classed({
                    area: 1,
                    "d3-enter": 1
                }).on("mouseover", function(c) {
                    d3.event.stopPropagation(), d.classed("focus", 1), d3.select(this).classed("focused", 1), b(e, a.tooltipFormatClass(c.className))
                }).on("mouseout", function() {
                    d3.event.stopPropagation(), d.classed("focus", 0), d3.select(this).classed("focused", 0), b(e, !1)
                }), t.exit().classed("d3-exit", 1).transition().duration(a.transitionDuration).remove(), t.each(function(a, b) {
                    var c = {
                        "d3-exit": 0
                    };
                    c["radar-chart-serie" + b] = 1, a.className && (c[a.className] = 1), d3.select(this).classed(c)
                    ...
                    var u = d.selectAll("g.circle-group").data(c, a.axisJoin);
                    u.enter().append("g").classed({
                        "circle-group": 1,
                        "d3-enter": 1
                    }), u.exit().classed("d3-exit", 1).transition().duration(a.transitionDuration).remove(), u.each(function(a) {
                        var b = {
                            "d3-exit": 0
                        };
                        a.className && (b[a.className] = 1), d3.select(this).classed(b)
                    }).transition().duration(a.transitionDuration).each("start", function() {
                        d3.select(this).classed("d3-enter", 0)
                    });
                    var v = u.selectAll(".circle").data(function(a, b) {
                        return a.axes.map(function(a) {
                            return [a, b]
                        })
                    });
                    v.enter().append("circle").classed({
                        circle: 1,
                        "d3-enter": 1
                    }).on("mouseover", function(c) {
                        d3.event.stopPropagation(), b(e, a.tooltipFormatValue(c[0].value))
                    }).on("mouseout", function(a) {
                        d3.event.stopPropagation(), b(e, !1), d.classed("focus", 0)
                    }), v.exit().classed("d3-exit", 1).transition().duration(a.transitionDuration).remove(), v.each(function(a) {
                        var b = {
                            "d3-exit": 0
                        };
                        b["radar-chart-serie" + a[1]] = 1, d3.select(this).classed(b)
        ...
        return c.config = function(b) {
            return arguments.length ? (arguments.length > 1 ? a[arguments[0]] = arguments[1] : d3.entries(b || {}).forEach(function(b) {
                a[b.key] = b.value
            }), c) : a
        }, c
    },
